...
int MIME_decode_raw( FFGET_FILE *f, RIPMIME_output *unpack_metadata, struct MIMEH_header_info *hinfo, int keep )
{
    ...
    if (file_has_uuencode)
    {
        ...
        //result = UUENCODE_decode_uu(NULL, hinfo->filename, hinfo->uudec_name, sizeof(hinfo->uudec_name), decode_entire_file, keep );
        result = UUENCODE_decode_uu(NULL, full_decode_path, hinfo->uudec_name, sizeof(hinfo->uudec_name), decode_entire_file, keep, unpack_metadata, hinfo );
        if (result == -1)
        {
...
int MIME_decode_text( FFGET_FILE *f, RIPMIME_output *unpack_metadata, struct MIMEH_header_info *hinfo, int keep )
{
    ...
    if (file_has_uuencode)
    {
        ...
        //result = UUENCODE_decode_uu( NULL, info->filename, hinfo->uudec_name, sizeof(hinfo->uudec_name), 1, keep );
        ...
        result = UUENCODE_decode_uu( NULL, ffname, hinfo->uudec_name, sizeof(hinfo->uudec_name), 1, keep, unpack_metadata, hinfo );
        if (result == -1)
        {
            switch (uuencode_error) {
                case UUENCODE_STATUS_SHORT_FILE:
...
int MIME_doubleCR_decode( char *filename, RIPMIME_output *unpack_metadata, struct MIMEH_header_info *hinfo, int current_recursion_level )
{
    ...
    else if (UUENCODE_is_diskfile_uuencoded(h.filename))
    {
        ...
        result = UUENCODE_decode_uu(NULL, filename, h.uudec_name, _MIMEH_FILENAMELEN_MAX , 1, 1, unpack_metadata, hinfo );
...
int MIME_decode_encoding( FFGET_FILE *f, RIPMIME_output *unpack_metadata, struct MIMEH_header_info *hinfo, struct SS_object *ss )
{
    ...
    switch (hinfo->content_transfer_encoding)
    {
        ...
        case _CTRANS_ENCODING_UUENCODE:
            ...
            result = UUENCODE_decode_uu(f, hinfo->filename, hinfo->uudec_name, sizeof(hinfo->uudec_name), 0, keep, unpack_metadata, hinfo );
            glb.attachment_count += result;
